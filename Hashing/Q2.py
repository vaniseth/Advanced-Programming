...
# Create a hash table with linear probing that can hold up to 10,000 integers.
hash_table = HashTable(10000)

# Generate 5,000 random integers and insert them into the hash table.
for i in range(5000):
...
# Measure the time it takes to insert a random integer into the hash table.
start_time = time.time()
hash_table.insert(random.randint(1, 100000))
end_time = time.time()
...
for lf in load_factors:
    ...
    hash_table.insert(random.randint(1, 100000))
    end_time = time.time()
    print(f"Load factor {lf}: Insert time: {end_time - start_time} seconds")
